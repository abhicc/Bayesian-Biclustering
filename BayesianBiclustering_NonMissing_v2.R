#==========================================================================================
#==========================================================================================
# This is the master code for the second version of the MCMC sampler for datasets with no missing entries
#==========================================================================================
#==========================================================================================

library(phyclust);library(reshape2);library(plyr);library(tidyverse);library(ggpubr);library(Rcpp);library(LaplacesDemon)
...
update_row_weight=function(reps)
{
  rwp=rep(NA,R);rw=rep(NA,R);for(i in 1:R){rwp[i]=(alpha/R)+sum(id_row_mat[,reps-1]==i)};rw=rdirichlet(1,rwp)
  return(list(weight=rw,weight_pars=rwp))
}

update_col_weight=function(reps)
{
  cwp=rep(NA,C);cw=rep(NA,C);for(i in 1:C){cwp[i]=(beta/C)+sum(id_col_mat[reps-1,]==i)};cw=rdirichlet(1,cwp)
  return(list(weight=cw,weight_pars=cwp))
}
...
row_id_update=function(dcol,rownum,reps)
{
  ...
  for(i in 1:R){mat[i,J+1]=row_weight[reps,i]*exp(a[i]-max(a))}
...
col_id_update=function(drow,colnum,reps)
{
  ...
  for(i in 1:C){mat[I+1,i]=col_weight[reps,i]*exp(a[i]-max(a))}
...
row_weight_pars=matrix(nrow=nreps+1,ncol=R);row_weight=matrix(nrow=nreps+1,ncol=R)
col_weight_pars=matrix(nrow=nreps+1,ncol=C);col_weight=matrix(nrow=nreps+1,ncol=C)
...
row_weight_pars[1,]=rep(alpha/R,R);row_weight[1,]=rdirichlet(1,row_weight_pars[1,])
col_weight_pars[1,]=rep(beta/C,C);col_weight[1,]=rdirichlet(1,col_weight_pars[1,])

#==========================================================================================
# second version of the Gibbs sampler
#==========================================================================================
for(reps in 2:(nreps+1))
{
  # update row and column weight vectors as in step 4(i)
  row_weights=update_row_weight(reps)
  row_weight[reps,]=row_weights$weight;row_weight_pars[reps,]=row_weights$weight_pars
  
  col_weights=update_col_weight(reps)
  col_weight[reps,]=col_weights$weight;col_weight_pars[reps,]=col_weights$weight_pars
  
  # row id updates as in step 4(ii)
  row_id_update_prob_mat=matrix(0,nrow=I,ncol=R)
  for(i in 1:I)
  {
    row_id_update_prob_mat[i,]=row_id_update(train_col,i,reps)
  }
  id_row_mat[,reps]=apply(row_id_update_prob_mat,1,function(x) sample(c(1:R),1,replace=TRUE,prob=x))
  train_row=cbind(train,idr=id_row_mat[,reps])
  
  # column id updates as in step 4(iii)
  col_id_update_prob_mat=matrix(0,nrow=J,ncol=C)
  for(j in 1:J)
  {
    col_id_update_prob_mat[j,]=col_id_update(train_row,j,reps)
  }
  id_col_mat[reps,]=apply(col_id_update_prob_mat,1,function(x) sample(c(1:C),1,replace=TRUE,prob=x))
  train_col=cbind(t(train),idc=id_col_mat[reps,])
  
  # update bicluster means as in step 4(iv)
...
g.ri.hist = ggplot(data=RI, aes(x=value)) + geom_histogram() + facet_grid(variable~.) + theme_bw() +
...
#==========================================================================================
save.image(file="BayesianBiclustering_NonMissing_v2.RData")
#==========================================================================================