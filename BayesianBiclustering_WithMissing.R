...
# minimum total SSE 
train_clust_1=train_original[order(id_row_mat[,which.min(sse_mat)]),order(id_col_mat[which.min(sse_mat),])]
...
v=cumsum(as.vector(table(id_col_mat[which.min(sse_mat),])))+0.5;vline_coords_1=data.frame(v=v[-length(v)])
h=cumsum(as.vector(table(id_row_mat[,which.min(sse_mat)])))+0.5;hline_coords_1=data.frame(h=h[-length(h)])
...
#==========================================================================================
# plot biclusters with predictions, biclusters determined by 3 criteria
#==========================================================================================
# minimum total SSE 
train_pred_clust_1=mat_observed_pred[order(id_row_mat[,which.min(sse_mat)]),order(id_col_mat[which.min(sse_mat),])]
